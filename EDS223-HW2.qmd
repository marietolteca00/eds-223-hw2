...
st_crs(map_in) #  User input: WGS 84 

st_crs(ej_df) #  User input: WGS 84 / Pseudo-Mercator 
...
if(st_crs(ej_df) != st_crs(birds)){
  warning("coordinate reference systems do not match :(")
}

# Transformation
birds <- st_transform(birds, crs = st_crs(ej_df))
...
```
```{r}
if(st_crs(ej_df) != st_crs(map_in)){
  warning("coordinate reference systems do not match :(")
}

map_in <- st_transform(map_in, crs = st_crs(ej_df))
...
mapin_ej_join <- st_join(ej_df, map_in, join = st_intersects)

# Creating Table
map_summary <- mapin_ej_join %>% 
  st_drop_geometry() %>% 
  group_by(grade) %>% 
